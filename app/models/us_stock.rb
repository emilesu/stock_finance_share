class UsStock < ApplicationRecord
  ...
  def us_stock_roe_ratio_pyramid
    array = JSON.parse(self.static_data)[13]        # 导入数据
    num_array = array.delete_if {|x| x == 0 }         # 去除空数据

    if num_array.size == 0 || (num_array.sum / num_array.size) == 0 || num_array.sum == 0 || num_array.min < 0 ||array.include?(0)
      rating = 0
  ...
  def us_stock_dividend_rate_pyramid
    array = JSON.parse(self.static_data)[32]        # 导入数据 分红率
    num_array = array.delete_if {|x| x == 0 }         # 去除空数据
  ...
  def us_stock_after_tax_profit_pyramid
    array = JSON.parse(self.static_data)[19]        # 导入数据 税后净利
  ...
  def us_stock_net_cash_flow_of_business_activities_pyramid
    array = JSON.parse(self.static_data)[24]        # 导入数据 经营活动现金流量
